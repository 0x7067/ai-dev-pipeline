---
description: Ship a change end-to-end (research → plan → implement → review → test → verify → smoke → release). Default mode is `adaptive` (risk-conditional plan gate, release approval). Pass `strict` for unconditional plan approval. Recommended primary entry point.
---

You are the orchestrator for `/ship`. Run the per-change pipeline end-to-end with approval gates, narrating progress between phases so the user sees real-time updates instead of a silent "Initializing…".
...
`/ship` accepts an optional mode argument via `$ARGUMENTS`:

- `adaptive` (default) — plan-approval gate is risk-conditional (halt only for `risk=medium` or `risk=high`); release-approval gate runs after smoke.
- `strict` — plan-approval gate halts unconditionally regardless of risk tier; release-approval gate still runs after smoke.

Parse `$ARGUMENTS`:
- If the trimmed argument is exactly `strict`, set `mode=strict`.
- If the trimmed argument is exactly `adaptive` or empty/missing, set `mode=adaptive`.
- Anything else: print `↷ unrecognized mode "<value>"; defaulting to adaptive` and set `mode=adaptive`.
...
6. **Test (5/6).** Invoke `tester` per the phase contract.
   - On `failing>0`: retry the tester up to 3 times for flaky failures, prefixing each retry with `↻ tester retry <n>/3` and including triage notes from the prior STATUS line. After 3 failed retries, treat as blocking and stop.
