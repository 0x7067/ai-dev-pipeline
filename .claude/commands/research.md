...
0. **Mint run-id and export environment.** Same protocol as `/ship` step 0:
   ...
   - Create `${RUN_DIR}/research/` and update `${AIDP_ARTIFACTS_ROOT}/latest`, `${AIDP_ARTIFACTS_ROOT}/latest.txt`, `${AIDP_PROJECT_ROOT}/.claude/workflow-state/active` atomically (`tmp + mv`).
